      ...
      * Berthier, L. & Barrat, J.L.  */
{
  ...
  static double alcada_L;
  static double alcada_R;

  int sys_size = p_sys_info->sys_size;
  if (snap == 0)
    {
      llistad_t * alcades_L = llistad_alloc ();
      llistad_t * alcades_R = llistad_alloc ();
	  ...
	  if (compleix_condicio_L (p_sys_info, sys, i)) 
	    {
	      ...
	      llistad_add (alcades_L, y);
	    }
	  if (compleix_condicio_R (p_sys_info, sys, i)) 
	    {
	      llistai_add (index_R, i);
	      double y = sys[index_y(i)];
	      llistad_add (alcades_R, y);
	    }
	}
      llistad_qsort (alcades_L, 0);
      llistad_qsort (alcades_R, 0);
      double y_min_L = llistad_min (alcades_L);
      double y_max_L = llistad_max (alcades_L);
      alcada_L = y_max_L - y_min_L;

      double y_min_R = llistad_min (alcades_R);
      double y_max_R = llistad_max (alcades_R);
      alcada_R = y_max_R - y_min_R;

      llistad_free (alcades_L);
      llistad_free (alcades_R);
    }
...
/*     } */
  ...
  int ny_extra = p_sys_info->ny_extra;
  int NY = p_sys_info->NY;
  int nx_paret = paret_size / (2 * ny_extra + NY);

  double normalitzacio_L = 1.;
  double normalitzacio_R = 1.;

  /* normalitzo amb particules  */
/*   if (n_particules_L != 0) normalitzacio_L = 1. / ((double) n_particules_L); */
/*   if (n_particules_R != 0) normalitzacio_R = 1. / ((double) n_particules_R); */
  
  /* poso pesos de paret, que no compten vaja  */
/*   normalitzacio_L = ((double) nx_paret) * normalitzacio_L; */
/*   normalitzacio_R = ((double) nx_paret) * normalitzacio_R; */

  /* normalitzo amb alcades  */
  if (alcada_L != 0) normalitzacio_L = normalitzacio_L / alcada_L;
  if (alcada_R != 0) normalitzacio_R = normalitzacio_R / alcada_R;

  /* normalitzacio amb NY  */
/*   if (NY != 0) normalitzacio_L = normalitzacio_L / ((double) NY); */
/*   if (NY != 0) normalitzacio_R = normalitzacio_R / ((double) NY); */

  /* normalitzo  */
  forca_paretL_x = normalitzacio_L * forca_paretL_x;
  forca_paretL_y = normalitzacio_L * forca_paretL_y;
  
  forca_paretR_x = normalitzacio_R * forca_paretR_x;
  forca_paretR_y = normalitzacio_R * forca_paretR_y;
		     ...
		     const double * sys, const int i)
{
  ...
  double long_y = (y_max_geom - y_min_geom);
  double y_min = y_min_geom + 0.5 * (1 - PERCENT/100.) * long_y;
  double y_max = y_max_geom - 0.5 * (1 - PERCENT/100.) * long_y;

/*   fprintf (stderr, "y_min_geom: %f y_min: %f\n", y_min_geom, y_min); */
/*   fprintf (stderr, "y_max_geom: %f y_max: %f\n", y_max_geom, y_max); */

  if ((i < phys_size) || (i >= phys_size + paret_size)) return 0;
  
  double y = sys[index_y(i)];
  if (( y > y_min) && (y < y_max)) return 1;
